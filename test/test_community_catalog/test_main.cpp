...
class TrackingMemory final:public services::ICommunityCatalogMemory {
 public:
  ...
  void* allocate(services::CommunityCatalogMemoryClass memory_class,std::size_t bytes) override{
    ...
    allocations.push_back({memory,bytes,memory_class});if(memory_class==services::CommunityCatalogMemoryClass::internal)++internal_allocations;else ++external_allocations;return memory;
  }
  ...
  bool check_heap(const void*) const override{return heap_valid;}
  ...
  std::vector<Allocation> allocations;unsigned internal_allocations{},external_allocations{},internal_releases{},external_releases{};bool fail_internal{},fail_external{},misclassify_internal{},heap_valid{true};mutable std::uint32_t clock{};
};
...
void valid_inflate_uses_external_buffers_and_internal_state(){Store store;TrackingMemory memory;services::CommunityCatalog catalog(store,&memory);auto result=catalog.search("sunlu",0);TEST_ASSERT_TRUE(result.ok());TEST_ASSERT_TRUE(memory.external_allocations>=2);TEST_ASSERT_TRUE(memory.internal_allocations>=1);TEST_ASSERT_EQUAL(memory.external_allocations,memory.external_releases);TEST_ASSERT_EQUAL(memory.internal_allocations,memory.internal_releases);TEST_ASSERT_TRUE(memory.allocations.empty());}
