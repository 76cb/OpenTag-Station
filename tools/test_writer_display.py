...
let communityMode='success',communityRelease;const communityRow={id:'brand-pla',name:'Community PLA',manufacturer:'Brand',material:'PLA',density:1.24,diameter:1.75};
...
window.fetch=async(path,options={})=>{if(String(path).includes('SpoolmanDB-Community/filaments.json')){if(communityMode==='hold')await new Promise(r=>communityRelease=r);if(communityMode==='failure')throw Error('Fixture network failure');return new Response(JSON.stringify([communityRow]));}if(window.fixtureReply&&!String(path).endsWith('/config'))return window.fixtureReply(path,options);if(!String(path).endsWith('/config'))throw Error('Fixture blocked unexpected request: '+path);return new Response(JSON.stringify({api_version:'v1',ok:true,data:fixtureConfig}),{headers:{'Content-Type':'application/json'}});};
"""
HOST = r"""
...
"""
CHECKS = r"""
window.OpenTagWriter.bind();window.OpenTagWriter.bindWriter();const T=window.OpenTagWriter,A=window.__OpenTagTest;A.bindWeighAndClear();
...
check(shown('writer-review'),'preview visible');check(id('writer-detail').textContent.includes('NXP ICODE SLIX2'),'tag type retained in Advanced');check(id('writer-diff').textContent.includes('1000 g'),'readable proposal');check(id('writer-tag').textContent.includes('E0:04:01:08:66:27:D8:D4'),'readable UID');check(!id('writer-advanced').open,'Advanced collapsed');check(!id('writer-notices').open,'metadata collapsed');check(id('writer-critical').textContent.includes('not atomic'),'critical warning prominent');check(shown('writer-confirm')&&!shown('writer-import')&&!shown('writer-retry'),'one applicable action');
...
// Exercise browser-side Community fetch while retaining the actual production CSP.
T.writerState.snapshot={phase:'idle'};T.writerState.step=1;await T.openModal();id('writer-search').value='';id('writer-source').value='community';communityMode='hold';const loading=T.writerSearch(false);check(id('writer-progress').textContent.includes('Loading SpoolmanDB Community'),'Community loading');check(!shown('writer-range')&&!shown('writer-pages'),'loading hides range and pages');check(!shown('writer-browse'),'Community hides Spools browse');check(!id('writer-results').textContent.includes('No matching'),'loading has no empty contradiction');communityRelease();await loading;check(id('writer-range').textContent.includes('of 1'),'Community filtered count');check(id('writer-results').textContent.includes('Community PLA'),'Community success');
T.writerState.community=null;communityMode='failure';await T.writerSearch(false);check(shown('writer-community-retry'),'Community retry visible');check(id('writer-progress').textContent.includes('Unable to load SpoolmanDB Community'),'persistent Community failure');check(!shown('writer-pages'),'failure hides stale pagination');communityMode='success';id('writer-community-retry').click();await new Promise(r=>setTimeout(r,100));check(id('writer-results').textContent.includes('Community PLA'),'retry succeeds');
...
const policy=document.querySelector('meta[http-equiv="Content-Security-Policy"]').content,connect=policy.match(/connect-src ([^;]+)/)[1];check(connect.includes('https://icezaza2543.github.io')&&!connect.includes('*')&&!connect.split(' ').includes('https:'),'exact production CSP origin');
const violations=[];document.addEventListener('securitypolicyviolation',e=>violations.push(e.blockedURI));await window.cspFetch('https://unrelated.invalid/opentag-fixture').catch(()=>{});await new Promise(r=>setTimeout(r,25));check(violations.some(v=>v.includes('unrelated.invalid')),'real CSP denies unrelated fetch');const controller=new AbortController();const deadline=setTimeout(()=>controller.abort(),100);await window.cspFetch('https://icezaza2543.github.io/SpoolmanDB-Community/filaments.json',{signal:controller.signal}).catch(()=>{});clearTimeout(deadline);check(!violations.some(v=>v.includes('icezaza2543.github.io')),'real CSP permits Community fetch attempt');
        ...
        try:
            for width in (1440, 1280, 1024, 768, 390):
                ...
                result = subprocess.run([chrome, '--headless', '--no-sandbox', '--disable-gpu',
                    '--no-proxy-server', '--host-resolver-rules=MAP icezaza2543.github.io 127.0.0.1, MAP unrelated.invalid 127.0.0.1', '--virtual-time-budget=12000', '--dump-dom',
