...
test('writer spool details show known zero and canonical weight values',()=>{const a=writerApp();writerChoose(a);const text=nodeText(a.document.getElementById('writer-selected-detail'));for(const expected of ['1000 g','0 g','130 g','1.75 mm','1.24 g/cm³'])assert.ok(text.includes(expected));});
...
test('writer nominal weight edit preserves unknown color and other unmodified values',async()=>{let result={};const a=writerApp({fetch:async(url,init)=>{if(init.method==='POST'){const b=JSON.parse(init.body);assert.deepEqual(b.changes,{weight:1000});result={phase:'updated',filament:{...SUNLU.filament,color_hex:null}};return jsonResponse(202,{operation_id:42});}return jsonResponse(200,String(url).includes('/operations/')?{id:42,state:'succeeded'}:result);}});a.T.selected(null,{...SUNLU.filament,weight:777.12,color_hex:null});a.T.openEditor('filament');a.document.getElementById('writer-editor-fields').querySelector('[name="weight"]').value='1000';await drivePromise(a.T.saveEditor(),a.clock);assert.equal(a.T.writerState.editor,null);assert.equal(a.T.writerState.material.weight,1000);});
