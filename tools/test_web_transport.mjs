...
test('writer Community page includes filtered total and source label',async()=>{const a=writerApp();a.document.getElementById('writer-source').value='community';a.T.writerState.community=Array.from({length:17},(_,i)=>({id:String(i),name:'Black',manufacturer:'Sunlu',material:'PLA',diameter:1.75,density:1.24}));await a.T.writerSearch(false);await a.T.writerSearch(true);assert.equal(a.document.getElementById('writer-range').textContent,'Showing 9–16 of 17');assert.match(nodeText(a.document.getElementById('writer-results').children[0]),/COMMUNITY — NOT YET IN SPOOLMAN/);await a.T.writerSearch(true);assert.equal(a.document.getElementById('writer-next').disabled,true);});
...
const COMMUNITY_URL='https://icezaza2543.github.io/SpoolmanDB-Community/filaments.json';
const COMMUNITY_ROW={id:'brand-pla-black',manufacturer:'Brand',name:'PLA Black',material:'PLA',density:1.24,diameter:1.75};
function catalogResponse(data=[COMMUNITY_ROW],options={}){return new Response(typeof data==='string'?data:JSON.stringify(data),{status:options.status||200,headers:options.headers||{}});}
test('production CSP narrowly permits pinned Community origin',()=>{const source=readFileSync(new URL('../src/web/local_web_server.cpp',import.meta.url),'utf8');const connect=source.match(/connect-src ([^;]+);/)[1];assert.ok(connect.split(' ').includes('https://icezaza2543.github.io'));assert.ok(!connect.includes('*'));assert.ok(!connect.split(' ').includes('https:'));assert.ok(!connect.includes('https://unrelated.invalid'));assert.ok(readFileSync(new URL('../src/web/web_assets.cpp',import.meta.url),'utf8').includes(COMMUNITY_URL));});
test('Community loading hides old results range pagination and Browse Spools',async()=>{let resolve;const a=writerApp({fetch:()=>new Promise(r=>resolve=r)});a.document.getElementById('writer-source').value='community';a.T.writerResults([SUNLU],'spool');const pending=a.T.writerSearch(false);assert.match(a.document.getElementById('writer-progress').textContent,/Loading SpoolmanDB Community/);assert.equal(displayed(a,'writer-range'),false);assert.equal(displayed(a,'writer-pages'),false);assert.equal(displayed(a,'writer-browse'),false);assert.doesNotMatch(nodeText(a.document.getElementById('writer-results')),/No matching|Sunlu/);resolve(catalogResponse());await pending;assert.equal(displayed(a,'writer-range'),true);assert.match(a.document.getElementById('writer-range').textContent,/of 1/);assert.doesNotMatch(a.document.getElementById('writer-progress').textContent,/Loading/);});
test('Community HTTP malformed and oversized failures persist with retry',async()=>{for(const response of [()=>catalogResponse('down',{status:503}),()=>catalogResponse('{bad'),()=>catalogResponse([],{headers:{'Content-Length':'67108865'}})]){let attempts=0;const a=writerApp({fetch:async url=>{assert.equal(url,COMMUNITY_URL);return attempts++?catalogResponse():response();}});a.document.getElementById('writer-source').value='community';await a.T.writerSearch(false);assert.match(a.document.getElementById('writer-progress').textContent,/Unable to load SpoolmanDB Community/);assert.equal(displayed(a,'writer-community-retry'),true);assert.equal(displayed(a,'writer-pages'),false);await a.T.writerSearch(false);assert.equal(attempts,2);assert.equal(displayed(a,'writer-community-retry'),false);assert.equal(a.T.writerState.items.length,1);}});
test('Community abort timeout is bounded and retryable',async()=>{const a=writerApp({fetch:async(url,options)=>new Promise((resolve,reject)=>options.signal.addEventListener('abort',()=>reject(Error('aborted'))))});a.document.getElementById('writer-source').value='community';const pending=a.T.writerSearch(false);a.clock.tick(30000);await pending;assert.match(a.document.getElementById('writer-progress').textContent,/timed out after 30 seconds/);assert.equal(displayed(a,'writer-community-retry'),true);});
test('Community stale async success and failure cannot replace switched source',async()=>{for(const fail of [false,true]){let resolve,reject;const a=writerApp({fetch:()=>new Promise((r,j)=>{resolve=r;reject=j;})});a.document.getElementById('writer-source').value='community';const pending=a.T.writerSearch(false);a.document.getElementById('writer-source').value='spoolman';a.T.writerState.queryVersion++;a.T.writerResults([SUNLU],'spool');if(fail)reject(Error('old failure'));else resolve(catalogResponse());await pending;assert.equal(a.T.writerState.entity,'spool');assert.match(nodeText(a.document.getElementById('writer-results')),/Sunlu/);assert.doesNotMatch(a.document.getElementById('writer-progress').textContent,/old failure/);}});
...
test('clear pending offers unlink retry and locks Escape during physical work',()=>{const a=loadApplication();a.T.bindWeighAndClear();a.document.getElementById('clear-dialog').showModal();a.T.renderClear({phase:'clearing',uid:'E00401086627D8D4',total_blocks:23,completed_blocks:14});a.T.closeClear();assert.equal(a.document.getElementById('clear-dialog').open,true);assert.match(a.document.getElementById('clear-message').textContent,/14 \/ 23 changed blocks verified/);a.T.renderClear({phase:'unlink_pending',uid:'E00401086627D8D4',message:'Tag is blank and verified. Spoolman unlink is still pending.'});assert.equal(a.document.getElementById('clear-retry').hidden,false);assert.equal(a.document.getElementById('clear-confirm').hidden,true);a.T.closeClear();assert.equal(a.document.getElementById('clear-dialog').open,false);assert.equal(a.document.activeElement.id,'clear-open');});
