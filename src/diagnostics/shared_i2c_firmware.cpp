...
constexpr std::uint32_t initialization_reference_checksum = 0xE99B8F04U;
...
<body><main>
  ...
  <script>
    ...
    const initLabels={state:'Initialization state',uid:'Authorized UID',uid_after_write:'UID after write',block_count:'Physical blocks',block_size:'Block size',
      ...
      mime_type:'MIME type',auxiliary_region:'Auxiliary region',blank:'Writable range blank',
 ...
 private:
    ...
    if (field_on != ERR_NONE) {
      ...
      operation_ok = [&]() {
        ...
        if (!decoded.ok() ||
            ...
            decoded.value().envelope.meta.used_size != 1U ||
  ...
  static esp_err_t preview_handler(httpd_req_t* request) {
    ...
    append_json(
        ...
        "\"mime_type\":\"%s\",\"auxiliary_region\":\"OMITTED\","
  ...
  static esp_err_t initialization_image_handler(httpd_req_t* request) {
    ...
    httpd_resp_set_hdr(
        request,
        "Content-Disposition",
        "attachment; filename=openprinttag-nfcv-initialization-312.bin");
    httpd_resp_set_hdr(request, "X-OpenPrintTag-Reference",
                       opentag::nfc::openprinttag::initializer_reference_revision);
    httpd_resp_set_hdr(request, "X-OpenTag-Checksum-FNV1A32", "E99B8F04");
