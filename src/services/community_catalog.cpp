...
core::Result<network::ResponseBody> inflate(ICommunityCatalogFile& file,const Directory& d){
  using Result=core::Result<network::ResponseBody>;network::ResponseBody input(block_limit),output(block_limit);
  if(!input.resize_uninitialized(d.compressed)||!output.resize_uninitialized(d.expanded))return Result::failure({core::ErrorCategory::backend_unavailable,"Community catalog memory unavailable",true});
  if(!file.read(d.offset,input.mutable_data(),d.compressed))return Result::failure(invalid("Community catalog block truncated"));
  auto inflater=network::make_external<tinfl_decompressor>([]{return tinfl_decompressor{};},"community_inflater");
  if(!inflater)return Result::failure({core::ErrorCategory::backend_unavailable,"Community catalog inflater memory unavailable",true});
  tinfl_init(inflater.get());std::size_t input_size=d.compressed,output_size=d.expanded;
  const auto status=tinfl_decompress(inflater.get(),reinterpret_cast<const mz_uint8*>(input.data()),&input_size,reinterpret_cast<mz_uint8*>(output.mutable_data()),reinterpret_cast<mz_uint8*>(output.mutable_data()),&output_size,TINFL_FLAG_USING_NON_WRAPPING_OUTPUT_BUF);
  if(status!=TINFL_STATUS_DONE||input_size!=d.compressed||output_size!=d.expanded||crc32(reinterpret_cast<const std::uint8_t*>(output.data()),d.expanded)!=d.crc)return Result::failure(invalid("Community catalog block damaged"));
