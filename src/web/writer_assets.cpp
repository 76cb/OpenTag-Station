...
window.OpenTagWriter={bind:function(){const {byId,asObject,asArray,first,setText,setValue,valueOf,showToast,api,submitMutation,PRIORITY}=window.OpenTagWriterHost;
  ...
  async function saveEditor(){if(!S.editor||S.busy)return;const edit=S.editor;try{const changes=formValues('writer-editor-fields',fields[edit.kind],edit.record);if(!Object.keys(changes).length){setText('writer-editor-message','No changed values to save.');return;}const body={action:'update_'+edit.kind,changes};body[edit.kind+'_id']=edit.record.id;if(edit.kind==='filament'&&S.spool)body.spool_id=S.spool;if(await writerCommand(body))closeEditor();else setText('writer-editor-message','Save was not verified. Displayed values remain the last verified data; refresh before retrying.');}catch(e){setText('writer-editor-message',e.message);}}
  ...
  function renderWriter(data){const v=asObject(data);S.snapshot=v;
    ...
    setText('writer-progress',String(v.message||({catalog:'Select an item to review its canonical data.',preview:'Review changes, then confirm this exact tag.',complete:'Write complete. Tag and Spoolman association verified.'}[v.phase]||v.phase||'Select a spool to begin.'))+(v.phase==='writing'?' ('+v.completed_blocks+'/'+v.total_blocks+' blocks)':''));
